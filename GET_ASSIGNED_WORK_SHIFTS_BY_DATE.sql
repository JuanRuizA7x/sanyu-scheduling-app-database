CREATE OR REPLACE PROCEDURE GET_ASSIGNED_WORK_SHIFTS_BY_DATE(
    P_DATE IN DATE,
...
BEGIN
    ...
    INSERT INTO WORK_SHIFT_REPORT_TEMP (WORK_SHIFT_ID, SCHEDULE_ID, USER_ID, IS_STARTED)
    SELECT WORK_SHIFT_ID, SCHEDULE_ID, USER_ID, CASE WHEN IS_STARTED = 1 THEN 'Sí' ELSE 'No' END
    FROM WORK_SHIFT
    WHERE "DATE" = P_DATE;

    UPDATE WORK_SHIFT_REPORT_TEMP T
    SET (
        ROLE_ID,
    ...
    SET IDENTIFICATION_TYPE = (
        SELECT NAME FROM IDENTIFICATION_TYPE I
    ...
    SET (EXTENSION_START_TIME, EXTENSION_END_TIME, EXTENSION_REASON) = (
        ...
        FROM SCHEDULE_EXTENSION S
        WHERE S.WORK_SHIFT_ID = T.WORK_SHIFT_ID
        FETCH FIRST 1 ROW ONLY
    );

    SELECT WORK_SHIFT_REPORT_TYPE(
        IDENTIFICATION_TYPE,
        IDENTIFICATION_NUMBER,
        FULL_NAME,
        EMAIL,
